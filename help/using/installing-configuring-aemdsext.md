...
* Installerar Document Security Extension för Microsoft Office
...
Det finns separata installationsprogram för 32- och 64-bitarsversioner av Microsoft Office. För 32-bitarsversionen av Microsoft Office hämtar du DocumentSecurityExtensionforMicrosoftOffice.exe. För 64-bitarsversionen av Microsoft Office hämtar du DocumentSecurityExtensionforMicrosoftOffice64.exe.
...
Du kan förkonfigurera Document Security Extension för Microsoft Office så att det pekar på en LiveCycles- eller AEM server så att användare som installerar Document Security Extension för Microsoft Office kan använda funktionerna utan att konfigurera någon anslutning. Användarna kan öppna skyddade dokument utan att behöva göra någon konfiguration. De kan dock inte skydda nya dokument förrän de konfigurerar klienten att använda en viss server.

I följande steg beskrivs hur du skapar och konfigurerar en MSI-fil. Den här MSI-filen innehåller de registervärden som krävs för att förkonfigurera Document Security Extension för Microsoft Office-installationsprogrammet till LiveCyclet eller AEM server som är installerad i ditt företag.
...
* Tillåt användare att spara en fil i oskyddat format när de inte kan ansluta till servern. Tack vare den här flexibiliteten kan du ta hänsyn till fall när användare skapar dokument när de inte är anslutna till nätverket (t.ex. när de befinner sig på ett flygplan).
...
Innan du börjar förkonfigurerar du installationsprogrammet så att det pekar på LiveCyclet eller AEM formulärservern, vilket beskrivs ovan i den här artikeln.
...
Mer information om hur du konfigurerar principen för automatisk tillämpning finns i [Konfigurera ett automatiskt program med standardprincipen](installing-configuring-aemdsext.md#p-configuring-automatic-application-of-a-default-policy-p).
...
1. Skapa ett DWORD **WatermarkMode** i registernyckeln WatermarkMode och ange DWORD-värdet **WatermarkMode** till **1**.
...
1. Öppna Microsoft Visual Studio. Bläddra och öppna filen CommonResources.dll för redigering.
   ...
   >[!NOTE]
   >
   >Om filen inte visas i lösningsutforskaren öppnar du filen igen med alternativet Öppna med. Välj resursredigeraren som redigerare.
...
1. **(För AEM Document Security-tillägget för Microsoft Office-installationsprogrammet med endast tillägget .exe)** Ersätt följande kodrad:
...
Det självextraherande installationsprogrammet AEM Document Security-tillägget för Microsoft Office paketerar filen custom CommonResources.dll och är klart för distribution.
